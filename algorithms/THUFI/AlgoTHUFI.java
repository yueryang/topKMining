...
public class AlgoTHUFI
{
	...
	public AlgoTHUFI(int top, double alpha, double beta) {
		this.topkstatic = top;
		this.alpha = alpha;
		this.beta = beta;
	}
	...
	{
		System.out.println("The THUFI algorithm has started. \n");
		maxMemory = 0;
		...
		thufi(itemsetBuffer, 0, null, listOfUtilityLists);
		checkMemory();
	...
	public void updateLeafprune(int i, Pair pair, List<Pair> revisedTransaction, List<UtilityList> ULs)
	{
		...
		for (int j = i - 1; j >= 0; --j)
		{
			...
			if (ULs.get(--followingItemIdx).item != pairAfter.item)
				break;
			Double twuItem1 = new Double(0);
			Double twuItem2 = new Double(0);
	...
	public void writeResultTofileUnord() throws IOException
	{
		...
		while (iter.hasNext())
		{
			...
			buffer.append(" #UTIL: "); // write separator
			buffer.append(pattern.utility); // write support
	 ...
	 * @throws IOException
	 */
	private void thufi(int[] prefix, int prefixLength, UtilityList pUL, List<UtilityList> ULs) throws IOException
	{
		for (int i = ULs.size() - 1; i >= 0; --i)
		...
		for (int i = ULs.size() - 2; i >= 0; --i)
		{
			...
			if (X.sumIutils + X.sumRutils >= minTopKValue && X.sumIutils > 0)
			{
				...
				thufi(prefix, prefixLength + 1, X, exULs);
			}
		}
	}
