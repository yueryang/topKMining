...
public class RunTTFE
{
	...
	public static void main(String[] args)
	{
		...
		if (runSample)
		{
			...
			boolean raise[] = { false, true, true, false, true, true }, isPrint = true;
			
			AlgoTTFE ttfe = new AlgoTTFE.Builder().buildDatabase("ttfe").buildTopK(topK).buildAlphaBeta(alpha, beta).buildRaise(raise).buildIsPrint(isPrint).build();
			try
			{
		...
		if (runExperiments)
		{
			String inputFolderPath = "./input/", outputFolderPath = "./output8/", inputFileExt = ".txt", outputFileExt = ".txt";
			String[] databaseList = { "accidents", "chess", "kosorak", "mushroom" };
			int[] topKList = { 10, 50, 100, 500, 1000 };
			double[] betaList = { 0, 0.25, 0.5, 0.75, 1 };
			boolean raise[] = { false, true, true, false, true, true }, isPrint = false;
					...
					if (inputFilePointer.isFile())
						for (int topK : topKList)
							for (double beta : betaList)
							{
								double alpha = 1 - beta;
								++runCount;
								AlgoTTFE ttfe = new AlgoTTFE.Builder().buildDatabase(database).buildTopK(topK).buildAlphaBeta(alpha, beta).buildRaise(raise).buildIsPrint(isPrint).build();
								try
								{
									ttfe.runAlgorithm(inputFilePath, outputFilePath);
									ttfe.printStats(null);
